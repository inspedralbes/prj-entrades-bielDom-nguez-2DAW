...
### Catàleg Ticketmaster Discovery i administració (sessió 2026-04-11)

- **Font de cerca al públic (Buscador)**: el llistat i filtres consulten **`events` / `venues` a PostgreSQL** (catàleg local). Ticketmaster **Discovery API** és la **font d’ingesta** per omplir o ampliar aquest catàleg, **no** una crida en línia obligatòria per cada cerca d’usuari (alineat amb **FR-017**: el backend pot servir dades ja sincronitzades).
- **Import en arrencada del stack (Docker / dev)**: en el **primer arrencament** del compose de desenvolupament (o script documentat equivalent), s’ha d’executar un **carregament inicial** d’esdeveniments des de Discovery cap al domini local (venues amb identificador extern TM quan escaigui, events amb `external_tm_id`), per tenir dades consistents sense passos manuals extres.
- **Job diari (sincronització TM)**: una vegada al dia (o interval equivalent al pla d’operacions), el backend consulta Discovery i **només insereix esdeveniments nous** (identificador TM encara absent a la taula local). Si un `external_tm_id` **ja existeix**, el job **no** ha d’actualitzar títols, dates ni altres camps des de TM: les correccions i el manteniment del catàleg local són responsabilitat de l’**administrador** (evita sobreescriure edicions manuals o polítiques de visibilitat).
- **Administració (CRUD esdeveniments)**: l’**Administrador** disposa de **gestió completa** sobre esdeveniments locals (crear, llegir, actualitzar, ocultar, pausar sincronització TM per fila si escau mitjançant `tm_sync_paused`, etc.). El panell **no** depèn del sync automàtic per conservar l’estat editat.
- **Implementació al repositori (referència)**: servei d’import Discovery (`TicketmasterDiscoveryEventsClient`, `TicketmasterEventImportService`), comanda Artisan `ticketmaster:sync-events`, programació al scheduler, endpoint admin `POST /api/admin/discovery/sync`, camps `venues.external_tm_id`, `events.tm_sync_paused`, i `PATCH /api/admin/events/{id}` per a camps administratius; l’evolució cap a **insert-only estricta** al job diari i **bootstrap Docker** queda com a refinament alineat amb aquesta sessió.
...
- **FR-017 (Buscador — llista)**: Ha d’existir una vista de **cerca per text**, **filtres de classificació** (p. ex. música, esports), **dates** i **rang de preu**; el catàleg es serveix des de **PostgreSQL** (esdeveniments **sincronitzats** des de Ticketmaster **Discovery API** al **backend**, vegeu **Catàleg Ticketmaster Discovery i administració (sessió 2026-04-11)**); opcionalment es pot afegir proxy en temps real a Discovery per a funcions addicionals; **botó flotant «Mapa»** obre la vista mapa; **icona de cor** desa l’esdeveniment a **Guardats**.
